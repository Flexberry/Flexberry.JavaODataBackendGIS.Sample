...
export default Ember.Mixin.create(LeafletZoomToFeatureMixin, {
    ...
    addToFavorite(feature) {
      ...
      if (Ember.get(feature.properties, 'isFavorite')) {
        if (layerModelIndex !== -1) {
          let records = store.peekAll('new-platform-flexberry-favorite-feature')
        ...
        let record = store.createRecord('new-platform-flexberry-favorite-feature', newRecord);
  ...
  */
  fromIdArrayToFeatureArray() {
    let store = this.get('store');
    let idFeaturesArray = store.findAll('new-platform-flexberry-favorite-feature');
